...
- Save plot comparing Exact vs Hybrid Annealer solution; optionally classical baseline.

Requirements:
- numpy, scipy (for optional classical baseline), matplotlib, requests
"""
...
# ---------------------------
# Automatski client (from your snippet, lightly wrapped)
# ---------------------------
...
# ---------------------------
# Utilities for decoding & residuals
# ---------------------------
...
# ---------------------------
# Optional classical baseline (root-based)
# ---------------------------

def classical_nonlinear_baseline(N=50):
    from scipy.optimize import root
    x = chebyshev_nodes(N, A, B)
    D = chebyshev_differentiation_matrix(N, A, B)

    def residual(u_inner):
        u = np.concatenate(([alpha], u_inner, [beta]))
        Du = D @ u
        r = D @ (a_u(u) * Du) - f_rhs(x)
        r[0]  = u[0]  - alpha
        r[-1] = u[-1] - beta
        return r[1:-1]

    # linear initial guess
    u0 = u_base_linear(x, A, B, alpha, beta)
    res = root(residual, u0[1:-1], method='hybr')
    u = np.concatenate(([alpha], res.x, [beta]))
    return x, u
...
def run_hybrid_picard_annealer(
    ...
    if with_classical_baseline:
        try:
            xc, uclass = classical_nonlinear_baseline(N=max(N, 33))
            # interpolate to x for fair visual
            uclass_i = np.interp(x, xc[::-1], uclass[::-1])
            plt.plot(x, uclass_i, "--", label="Nonlinear classical (root)")
        except Exception as e:
            print("[Warn ] classical baseline failed:", e)
